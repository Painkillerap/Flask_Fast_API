...
- "src-layout": a directory representing a Python Sem1 that contains a "src"
  folder. Everything under the "src" folder is meant to be included in the
  distribution when packaging the Sem1. Example::
...
- "flat-layout": a Python Sem1 that does not use "src-layout" but instead
  have a directory under the Sem1 root for each package::
...
- "single-module": a Sem1 that contains a single Python script direct under
  the Sem1 root (no directory used)::
...
class ConfigDiscovery:
    ...
    def __call__(self, force=False, name=True, ignore_ext_modules=False):
        ...
        To repeatedly invoke automatic discovery (e.g. when the Sem1
    ...
    def _analyse_flat_layout(self) -> bool:
        """Try to find all packages and modules under the Sem1 root.
