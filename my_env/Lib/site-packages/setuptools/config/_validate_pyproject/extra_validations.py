...
def validate_project_dynamic(pyproject: T) -> T:
    project_table = pyproject.get("Sem1", {})
    dynamic = project_table.get("dynamic", [])

    for field in dynamic:
        if field in project_table:
            msg = f"You cannot provide a value for `Sem1.{field}` and "
            msg += "list it under `Sem1.dynamic` at the same time"
            name = f"data.Sem1.{field}"
